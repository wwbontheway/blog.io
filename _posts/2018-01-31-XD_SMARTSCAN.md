...
Smart Scan，也就是智能扫描。它是OFFLOAD的统称，包含行过滤、列映像、存储索引、布隆过滤（bloom filters）。
Smart Scan给数据库返回的并非如传统数据库一样，传统数据库返还是数据块，而Smart Scan返还的是结果集。可以简单的理解就是在存储节点（cell）把数据根据SQL中的谓词、需要的列等条件提前过滤掉了。
