...
固定一个SQL的执行计划除了在SQL中添加HINT、绑定OUTLINE之外，也可以使用SQL PROFILE，这是Oracle 10g开始出现的一个新特性，这里提供一个官方的脚本，可以很方便的绑定执行计划，有了这个脚本就不需要自己在DBMS_SQLTUNE中手动拼写参数，如果短小的sql还好，要是很长的sql，光转义就累疯了，嘿嘿~

此脚本来自ORACLE COE团队,具体可以参考MOS文档“ID 215187.1”，脚本在SQLTXPLAIN的ZIP包(sqlt.zip)中，解压之后在utl目录下，名为coe_xfr_sql_profile.sql，可以快速的进行sql profile的绑定。需要待绑定SQL的sql_id，已经调试好的plan_hash_value（就是已经添加hint等方式调好的sql的执行计划），然后运行如下脚本，生成之后，需要先将生成脚本中的 force_match 改为TRUE在执行。

这系列博文将对这个脚本进行一些测试篇幅原因大概会分几篇去写，此篇就是系列第一篇~

## 脚本内容

本来不想直接把脚本放上来，但是考虑到很多朋友可能没有Oracle MOS账号，无法自行下载，所以还是花一部分篇幅来将脚本正文粘贴出来，这个脚本中其实注释很多，但是为了维护原作者的版权，我就完全照搬了。原谅我用这个脚本就水了一篇博文，嘿嘿~
...
以上就是脚本的正文全部内容，关于测试未来找时间会一一补全的，嘿嘿~~
